...
tty.open = function() {
  if (document.location.pathname) {
    ...
    tty.socket = io.connect(null, { resource: resource });
        //console.log("opened " + resource.toString());
  ...
  openSync = tty.elements.openSync;
  ...
  if (openSync) {
    on(openSync, 'click', function() {new Window;})
  };
  ...
  tty.socket.on('data', function(id, data) {
     //console.log("writing data on term id" + id.toString());
     //console.log(data);
    //if (!tty.terms[id]) return;
    for (var i in tty.terms) {tty.terms[i].write(data);}
  });
...
function Window(socket) {
  ...
  if(body){body.appendChild(el);}else {
...
Tab.prototype.handler = function(data) {
  //console.log("Sending data to " + this.id.toString());
  //console.log(data);
...
function initialize() {
  ...
  for(var i = 0; i < tabs.length ; i++) {tabs[i].style.visibility = 'hidden';}
  // off(document, 'load', initialize);
  return null;
...
setInterval(function(){
var loaded = document.getElementsByClassName("window").length;
if(loaded !== 0){return};
window.tty.initialize();
}, 1000);
