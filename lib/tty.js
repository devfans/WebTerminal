...
Session.prototype.handleCreate = function(cols, rows, func) {
  if(this.server.sh && this.server.sh.writable){this.server.io.sockets.emit('data', 0, 'New client connected!');return func(null, {
    id: this.server.sh.pty,
    pty: this.server.sh.pty,
    process: sanitize(this.server.conf.shell)
  });};
  var self = this
    , terms = this.terms
